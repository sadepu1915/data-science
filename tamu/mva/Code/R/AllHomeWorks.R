...
lasso_mean[te]<-((data_st[te,]$Life.expectancy-pred_lasso)^2)
}
...
pred_glm_cv <- predict(fit_glmodel_cv,newdata=te_data,type = 'response')
...
hof_lasso_truth_table<-table(te_data$HOF, hof_pred_lasso > 0.5)
...
setwd("~/MS/636/HW")
raw_data <- read.csv("./HW06/Life_Expectancy.csv")

vars <- c("Life.expectancy",  "Income.composition.of.resources", "Schooling")
no_na_data<-na.omit(as.matrix(raw_data[, vars]))
data_st <- data.frame(no_na_data[,1], no_na_data[,-1])
colnames(data_st) <- c("Life.expectancy", vars[-1])
...
#1Ai
fit_poly4<-lm(Life.expectancy~poly(Schooling,4), data=data_st)
coef(summary(fit_poly4))

#1Aii
fit_poly3<-lm(Life.expectancy~poly(Schooling,3), data=data_st)
school_grid <- seq(from = 1.53, to = 20.04)
pred_poly3 <- predict(fit_poly3, newdata=list(Schooling=school_grid), se=TRUE)
plot(data_st$Schooling, data_st$Life.expectancy)
lines(school_grid, pred_poly3$fit, lwd=3, col="green")
se.bands<-cbind(pred_poly3$fit+2*pred_poly3$se.fit, pred_poly3$fit-2*pred_poly3$se.fit)
matlines(school_grid, se.bands, lwd=3, col="red")
title("Degree 3 Poly with 2*error")
    
#1Bi

library(splines)
fit_cs<-lm(Life.expectancy~bs(Schooling, knots=c(5,10,15)), data=data_st)
pred_cs <- predict(fit_cs, newdata=list(Schooling=school_grid), se=TRUE)
plot(data_st$Schooling, data_st$Life.expectancy)
lines(school_grid, pred_cs$fit, lwd=3, col="green")
se.bands<-cbind(pred_cs$fit+2*pred_cs$se.fit, pred_cs$fit-2*pred_cs$se.fit)
matlines(school_grid, se.bands, lwd=3, col="red")
title("cubic splines")

#1Bii
fit_ns<-lm(Life.expectancy~ns(Schooling, df=5), data=data_st)
pred_ns <- predict(fit_ns, newdata=list(Schooling=school_grid), se=TRUE)
plot(data_st$Schooling, data_st$Life.expectancy)
lines(school_grid, pred_ns$fit, lwd=3, col="green")
se.bands<-cbind(pred_ns$fit+2*pred_ns$se.fit, pred_ns$fit-2*pred_ns$se.fit)
matlines(school_grid, se.bands, lwd=3, col="red")
title("natural splines")

#1Biii
ss<-smooth.spline(data_st$Schooling, data_st$Life.expectancy, cv=TRUE)
lambda<-ss$df
...
setwd("~/MS/636/HW")
raw_data <- read.csv("./HW06/winequality_red.csv")

qual_high <- ifelse(raw_data$quality >= 8,"Yes", "No") 
data_st <- data.frame(raw_data, qual_high)
...
#2Ai
library(tree)
wine_tree <- tree(qual_high~.-quality, data_st)
summary(wine_tree)

#2Aii
set.seed(1)
tr<-sample(1:nrow(data_st), 1000)
te<-data_st[-tr,]
sa_wine_tree <- tree(qual_high~.-quality, data_st,subset=tr)
sa_wine_tree_pred <- predict(sa_wine_tree, te, type="class")
table(sa_wine_tree_pred, qual_high[-tr])

#2Bi
library(randomForest)
set.seed(1)
rf_fit<-randomForest(qual_high~.-quality, data_st,subset=tr, mtry=sqrt(7), importance=TRUE)
rf_pred <- predict(rf_fit, newdata=te)
table(rf_pred, qual_high[-tr])
...
for(i in 1:bt)
{
  ...
  rf_fit<-randomForest(qual_high~.-quality, data_st,subset=tr, mtry=sqrt(7),importance=TRUE)
  rf_pred <- predict(rf_fit, newdata=te)
  MSE_boot[i] <- mean((rf_pred - te$qual_high)^2)
}
sd(MSE_boot)
