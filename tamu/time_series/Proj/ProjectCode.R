...
fit <- arima(dataLag$Lumber_diff[12:(nrow(dataLag)-12)], order=c(1,0,1))
fit #Fit shows that both AR terms and MA terms are NOT significant; AIC = 1023.04

#Fit ARIMA(0,0,1) to differenced data
fit2 <- arima(dataLag$Lumber_diff[12:(nrow(dataLag)-12)], order = c(0,0,1))
...
fit3 <- arima(dataLag$Lumber_diff[12:(nrow(dataLag)-12)], order = c(1,0,0))
...
#Based on the AIC, MA model looks better than AR model
...
lines(fore$pred - 1.96*fore$se, lty="dashed", col="green")
legend(0,-10, legend=c("Actual", "Fitted", "Forecast", "PI Bounds"),
