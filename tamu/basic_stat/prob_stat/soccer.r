...
results1=cbind(soccer [,3],round(dpois(0:9,lambda=mean(goals))*length(goals),1),round(dnbinom(0:9,nuhat,phat)*sum(soccer[,3]),1))
readline()
...
results2=cbind(soccer[,2],round(dpois(0:9,lambda=mean(passes))*length(passes),1),round(dnbinom(0:9,nuhat,phat)*sum(soccer[,2]),1))
colnames(results2)=c("Observed","Poisson","Negbin")
rownames(results2)=as.expression(0:9)
...
colnames(gofresults1)=c("Observed","Poisson","Negbin")
rownames(gofresults1)=as.expression(0:6)
poisgofstat1=sum((gofresults1[,1]-gofresults1[,2])**2/gofresults1[,2])
negbingofstat1=sum((gofresults1[,1]-gofresults1[,3])**2/gofresults1[,3])
gofresults1
poisgofstat1
negbingofstat1


gofresults2=rbind(results2[1:7,1:3],c(sum(results2[8:10,1]),sum(results2[,2])-sum(results2[1:7,2]),sum(results2[,3])-sum(results2[1:7,3])))

poisgofstat2=sum((gofresults2[,1]-gofresults2[,2])**2/gofresults2[,2])
negbingofstat2=sum((results2[,1]-results2[,3])**2/results2[,3])
results2
poisgofstat2
negbingofstat2
