...
datafile = os.path.join(os.getcwd(), "SheatherData/HoustonRealEstate.xlsx")
df   = pd.read_excel(datafile)
print(df[0:10], '\n')

# Fit Simple Linear Regression Model
model = sm.wls('Price ~ NewConst + Foreclosures', data=df, sample_weight=df['n'])
results = model.fit()
...
ax.plot(df['Price'], df['Predicted'], 'o', label="Predicted")
